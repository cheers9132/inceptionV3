...
# https://blog.csdn.net/qq_41660119/article/details/106126519

# 将卷积，BatchNormalization, Activation写成一个函数
def conv2d_bn(x,
              filters,
              num_row,
              num_col,
...
def InceptionV3(input_shape=[299, 299, 3],
    ...
    # valid方式发现余下的窗口不足时直接将多余列舍弃，而same方式会把多出的列填充0
    x = conv2d_bn(img_input, 32, 3, 3, strides=(2, 2), padding='valid')
    print('第1次卷积维度', x.shape)   # (149, 149, 32)
    x = conv2d_bn(x, 32, 3, 3, padding='valid')
    print('第2次卷积维度', x.shape)   # (147, 147, 32)
    x = conv2d_bn(x, 64, 3, 3)
    print('第3次卷积维度', x.shape)   # (147, 147, 64)    默认padding='same', 会自动填充保证 高 宽 不变
    x = MaxPooling2D((3, 3), strides=(2, 2))(x)
    print('第4层最大池化维度', x.shape)     # (73, 73, 64)    147/2 -> 73, 多余列舍弃

    x = conv2d_bn(x, 80, 1, 1, padding='valid')
    print('第5次卷积维度', x.shape)   # (73，73，80)
    x = conv2d_bn(x, 192, 3, 3, padding='valid')
    print('第6次卷积维度', x.shape)   # (71, 71, 192)
    x = MaxPooling2D((3, 3), strides=(2, 2))(x)
    print('第7次最大池化层', x.shape)      # (35, 35, 192)
    ...
    # conv_bn默认same填充方式，所以 高 宽 不变
    branch1x1 = conv2d_bn(x, 64, 1, 1)
    print("block1 part1 1x1", branch1x1.shape)

    branch5x5 = conv2d_bn(x, 48, 1, 1)
    branch5x5 = conv2d_bn(branch5x5, 64, 5, 5)
    print("block1 part1 5x5", branch5x5.shape)

    branch3x3dbl = conv2d_bn(x, 64, 1, 1)
    branch3x3dbl = conv2d_bn(branch3x3dbl, 96, 3, 3)
    branch3x3dbl = conv2d_bn(branch3x3dbl, 96, 3, 3)
    print("block1 part1 3x3", branch3x3dbl.shape)

    branch_pool = AveragePooling2D((3, 3), strides=(1, 1), padding='same')(x)
    branch_pool = conv2d_bn(branch_pool, 32, 1, 1)
    print("block1 part1 pool", branch_pool.shape)
...
if __name__ == '__main__':
    ...
    # np.expand_dims可以在数据的某一维上插入维度，使其具有固定的维度。
